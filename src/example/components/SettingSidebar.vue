  ...
  <div class="sidebar" :class="{ disable: !this.editor.enabled }">
    <div class="content" :class="{ 'has-actions': showActions }">
      ...
      <div v-else class="empty-view">
        ...
        <span class="description"> 在预览面板上点击一个元素，开始编辑。 </span>
      </div>
    </div>

    <div v-if="showActions" class="actions">
      <el-button @click="deleteNode" type="danger" icon="el-icon-delete" circle></el-button>
      <el-button @click="focusParent" icon="el-icon-top-right" circle></el-button>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    settingComponents() {
      ...
      console.log('setting', this.editor.getSettings(this.selectedNode));
  ...
  methods: {
    deleteNode() {
      this.$confirm('这将永久删除此元素。是否继续?', 'Warning', {
